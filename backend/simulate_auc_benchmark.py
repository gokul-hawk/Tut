...
# --- 2. DATA GENERATOR (Multi-Scenario Graph Random Walk) ---
def generate_student_data(n_students=50, n_steps=30, scenario="steady"):
    ...
    # Scenario Profiles (Probabilities for Logic: L=Learn, T=Transit, S=Slip, G=Guess)
    profiles = {
        "steady":    {"p_L": 0.1, "p_T": 0.1,  "p_S": 0.1, "p_G": 0.25},
        "expert":    {"p_L": 0.5, "p_T": 0.3,  "p_S": 0.05,"p_G": 0.25},
        "struggler": {"p_L": 0.05,"p_T": 0.02, "p_S": 0.3, "p_G": 0.1},
        "noisy":     {"p_L": 0.1, "p_T": 0.1,  "p_S": 0.4, "p_G": 0.4}
    }
    prof = profiles.get(scenario, profiles["steady"])
    
    dataset = []
    for _ in range(n_students):
        # Hidden Mastery State for 4 nodes
        hidden_states = np.array([prof["p_L"]] * 4)
        history = []
        
        for t in range(n_steps):
            # Curriculum traversal: Basics -> Intermediate -> Advanced
            if t < 10: node_idx = 0 # Basics
            elif t < 20: node_idx = random.choice([1, 2]) # Loops or Recursion
            else: node_idx = 3 # Debugging
            
            # Map node to source modality
            if node_idx == 0: source = "tutor"
            elif node_idx in [1, 2]: source = "code"
            else: source = "debug"
            
            # True mastery determines success probability (with Slip/Guess)
            ...
            if is_correct:
                ...
                # Propagation to children (simplified)
...
def run_auc_benchmark():
    ...
    full_dataset = []
    for sc in scenarios:
        full_dataset.extend(generate_student_data(n_students=50, n_steps=50, scenario=sc))
    
    print(f"Generated {len(full_dataset)} student traces.")
    sys.stdout.flush()
    ...
    try:
        for student in full_dataset:
            ...
            for i, event in enumerate(events):
                # Event format: (source, is_correct, node_idx)
                source, is_correct, node_idx = event
                ...
                for name, model in models.items():
                    ...
                    if name == "MGKT":
                        ...
                        # GKT typically predicts for the active node
                        pred = model.mastery
                    else:
                        pred = model.mastery
